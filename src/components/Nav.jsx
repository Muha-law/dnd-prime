  ...
  return (
    <nav className={"fixed top-0 left-0 right-0 z-50 backdrop-blur-[16px] transition-colors duration-300 " + bg}>
      ...
      <div className={"lg:hidden overflow-hidden transition-all duration-300 ease-in-out " + (menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0")}>
        <div className="bg-surface border-t border-line px-6 py-6 space-y-4">
          {links.map((l, i) => (
            <Link
              key={l.to}
              to={l.to}
              className="block text-base text-on-dark/90 transition-all duration-200"
              style={{ transitionDelay: menuOpen ? `${i * 40}ms` : "0ms", opacity: menuOpen ? 1 : 0, transform: menuOpen ? "translateY(0)" : "translateY(-6px)" }}
            >
              {l.label}
            </Link>
          ))}
          <Link
            to="/contact"
            className="block text-center bg-accent text-on-surface px-5 py-3 text-body-md font-semibold transition-all duration-200"
            style={{ transitionDelay: menuOpen ? `${links.length * 40}ms` : "0ms", opacity: menuOpen ? 1 : 0, transform: menuOpen ? "translateY(0)" : "translateY(-6px)" }}
          >
            Get a quote
          </Link>
        </div>
